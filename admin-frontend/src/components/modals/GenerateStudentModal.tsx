...
const CLASS_OPTIONS = ["Класс 6", "Класс 7"];
const LANGUAGE_OPTIONS = ["RU", "KG"];
...
export function GenerateStudentModal({
    ...
    // Простая генерация логина (транслит) и рандомного пароля
    ...
    const handlePreviewGeneration = () => {
        ...
        // Формат логина: firstname.lastname
        const generatedUsername = `${firstName}.${lastName}`.toLowerCase();

        // Простой пароль: 2 случайных слова + 3 цифры
        const randomNum = Math.floor(Math.random() * 900) + 100;
        const generatedPassword = `akyl${randomNum}`;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!fullName || !previewUsername || !previewPassword) return;
            ...
            <div className="bg-card w-full max-w-lg rounded-xl shadow-lg border border-border overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between p-6 border-b border-border">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-xl font-semibold text-foreground">Новый ученик</h2>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="langSelect">Язык</Label>
                                <select
                                    id="langSelect"
